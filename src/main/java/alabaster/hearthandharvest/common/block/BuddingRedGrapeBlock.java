...
public class BuddingRedGrapeBlock extends BuddingBushBlock implements BonemealableBlock
{
    public BuddingRedGrapeBlock(Properties properties) {
        super(properties);
    }
    ...
    public BlockState updateShape(BlockState state, Direction facing, BlockState facingState, LevelAccessor level, BlockPos currentPos, BlockPos facingPos) {
        if (state.getValue(BuddingBushBlock.AGE) == 4) {
            level.setBlock(currentPos, HHModBlocks.RED_GRAPE_CROP.get().defaultBlockState(), 3);
        }
    ...
    public void growPastMaxAge(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        level.setBlockAndUpdate(pos, HHModBlocks.RED_GRAPE_CROP.get().defaultBlockState());
    }
    ...
    public void performBonemeal(ServerLevel level, RandomSource random, BlockPos pos, BlockState state) {
        ...
        if (ageGrowth <= maxAge) {
            ...
            level.setBlockAndUpdate(pos, HHModBlocks.RED_GRAPE_CROP.get().defaultBlockState().setValue(RedGrapeVineBlock.VINE_AGE, remainingGrowth));
        }
    }
}